...
class Genea {
	...
	generationsSize: number;
	doneFitness: number;
	geneLength: number;
	getFitness: any;
	outOfGenerationsSize: any;
	...
	getMatches() {
		let bests = [];
		this.populations.forEach((individuals,i) => {
...
function run(str) {
	...
	const ga = new Genea({
		...
		onGeneration: function (generation,genes) {
			var max = 0;
			var index = 0;
			this.fitnesses.forEach((fitness,i) => {
				if (fitness > max) {
					max = fitness;
					index = i;
				}
...
function play_dom(){
	...
	const history = run('Too young,too simple. Sometimes, naive')
	isRunning = true;
	let i =0;
